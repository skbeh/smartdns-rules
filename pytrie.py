...
class Node(object):
    ...
    def numkeys(self):
        ...
        return int(self.value is not NULL) + sum(
            child.numkeys() for child in self.children.values()
        )
...
class Trie(MutableMapping):
    ...
    def iter_prefix_items(self, key):
        ...
        def iter_items(node, prefix):
            for key, value in node.items():
                new_prefix = (*prefix, key)
                yield (new_prefix, value.value if value.value is not NULL else None)
                if value.children != {}:
                    iter_items(value.children, new_prefix)
        ...
        for part in key:
            ...
            if node.value is not NULL:
                yield (key_factory(prefix), node.value)
                break
        yield from iter_items(node.children, prefix)
    ...
    def iteritems(self, prefix=None):
        ...
        def generator(
            node, key_factory=self.KeyFactory, parts=parts, append=append, null=NULL
        ):
            if node.value is not null:
