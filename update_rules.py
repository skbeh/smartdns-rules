    ...
    def split_single_syntax(raw_rule_line: str) -> Optional[Rule]:
        ...
        if application == "address":
            match destination:
                ...
                case "-":
                    return (RuleApplication.IGNORE_BLOCK, domain, None)
                case _:
        ...
        if application == "nameserver":
            ...
            if destination == "-":
                return (RuleApplication.IGNORE_REDIRECT, domain, None)
...
RULE_LINK_TUPLE: Final[tuple[RuleSet, ...]] = (
    ...
    SpecifiedHostsRule(
        "https://raw.githubusercontent.com/vokins/yhosts/master/data/moot/cps.txt",
        RuleApplication.IGNORE_BLOCK,
    ),
)
...
def encode_smartdns(
    ...
    if block_rule_list is not None:
        block_list += list(map(lambda rule: rule[0], join_domain(block_rule_list)))
        config_list.append("domain-set -name block -type list -file /tmp/block.list")
        config_list.append("address /domain-set:block/#")
    ...
    return config_list, block_list


def output(output_file: TextIOWrapper, block_file: TextIOWrapper) -> None:
    def extract_set(
        application: RuleApplication,
    ...
    def write_out(file: TextIOWrapper, rule_list: list[str]) -> None:
        rule_list.sort()
    ...
    ignore_block_rule_set = extract_set(RuleApplication.IGNORE_BLOCK, grouped_list)
    redirect_rule_set = extract_set(RuleApplication.REDIRECT, grouped_list)
    ignore_redirect_rule_set = extract_set(
        RuleApplication.IGNORE_REDIRECT, grouped_list
    )
    ...
    if block_rule_set is not None:
        ...
        if ignore_block_rule_set is not None:
            remove_ignore(block_rule_set, ignore_block_rule_set)
    ...
    if redirect_rule_set is not None:
        ...
        if ignore_redirect_rule_set is not None:
            remove_ignore(redirect_rule_set, ignore_redirect_rule_set)
    ...
    config_list, block_list = encode_smartdns(
    ...
    write_out(output_file, config_list)
    if block_list:
        write_out(block_file, block_list)
...
def main() -> None:
    ...
    output_file: TextIOWrapper = parser.parse_args().output_file
    block_file: TextIOWrapper = parser.parse_args().block_file

    asyncio.run(async_main())

    consume(map(process_rule, RULE_LINK_TUPLE))

    output(output_file, block_file)
